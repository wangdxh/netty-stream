 ...
 * </table>
 */
public class SipDecoder extends HttpObjectDecoder {
    /**
     ...
     */
    private static final Pattern versionPattern = Pattern.compile("SIP/\\d\\.\\d");
     ...
     */
    public SipDecoder(final int maxInitialLineLength,
    ...
    protected HttpMessage createMessage(final String[] initialLine)
        ...
        if (versionPattern.matcher(initialLine[0]).matches()) {
            ...
            return new DefaultHttpResponse(SipVersions.valueOf(initialLine[0]),
                new HttpResponseStatus(Integer.parseInt(initialLine[1]),
                                       initialLine[2]),
                validateHeaders);
        } else {
            isDecodingRequest = true;
            return new DefaultHttpRequest(SipVersions.valueOf(initialLine[2]),
                    SipMethods.valueOf(initialLine[0]),
                    initialLine[1],
                    validateHeaders);
        }
    }
    ...
    protected boolean isContentAlwaysEmpty(final HttpMessage msg) {
        ...
        // header is absent. and sip also is
        return super.isContentAlwaysEmpty(msg) || !msg.headers().contains(SipHeaderNames.CONTENT_LENGTH);
    }

    @Override
    protected HttpMessage createInvalidMessage() {
        if (isDecodingRequest) {
            return new DefaultFullHttpRequest(SipVersions.SIP_2_0,
                       SipMethods.INVITE, "bad-request", validateHeaders);
        } else {
            return new DefaultFullHttpResponse(SipVersions.SIP_2_0,
                                               UNKNOWN_STATUS,
                                               validateHeaders);
        }
    }
